...
您可以查看下面的示例目录，它描述了各种环境拓扑，包括
xref:multiple-masters[使用多个主机] 以获得可用性。
您可以选择一个匹配您需求的示例，修改它以匹配您自己的环境，并在
xref:running_install.adoc#running-the-installation-playbooks[运行安装] 将其作为目录文件。
...
下表中描述了单个
xref:../architecture/infrastructure_components/kubernetes_infrastructure.adoc#master[master]的运行环境。
...
你可以在以下示例目录文件的 *[masters]*, *[etcd]*, 和 *[nodes]* 小节中看到这些示例主机:
...
您可以在以下示例目录文件的 *[masters]*, *[nodes]*, 和 *[etcd]* 小节中看到这些示例主机:
...
* VIP监听指定的端口
xref:advanced-master-ports[`openshift_master_api_port`](默认情况下为8443) 
并代理回该端口上的所有主机。
* VIP在DNS中注册的域名。
...
要配置多个主机，请参考
xref:multi-masters-using-native-ha-ai[多个etcd与多个Master]
...
您可以在以下示例目录文件的 *[masters]*, *[etcd]*, *[lb]* 和 *[nodes]* 小节中看到这些示例主机:
