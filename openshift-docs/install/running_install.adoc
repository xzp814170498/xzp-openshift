...
要安装一个{product-title}集群，需要运行一系列可运行的剧本。
...
以及安装Ansible version 2.6或更高版本。您必须安装Ansible运行安装剧本。
...
您可以在云环境中提供{product-title} vm。您可以使用Ansible playbook自动化云托管基础设施的定义，
...
有关更多详细信息，请参见
链接:https://access.redhat.com/documentation/en-us/reference_architectures/2017/html-single/deploying_and_managing_red_hat_openshift_container_platform_3.6_on_red_hat_openstack_platform_10[参考架构]
用于{product-title} 3.6 和 Red Hat OpenStack Platform 10。 

作为使用OpenStack CLI的先决条件，首先提供VM并配置云基础设施，如网络、存储、防火墙和安全组。有关这些配置任务的信息，请参见
链接:prerequisites#prereq-cloud-provider-considerations[cloud provider considerations]
和链接:https://github.com/openshift/openshift-ansible/tree/master/playbooks/openstack[Ansible playbooks]
来自动化它们。请参见
...
基于RPM的安装程序使用通过RPM包安装的Ansible来运行本地主机上可用的剧本和配置文件。

[IMPORTANT]
====
不要在`nohup`下运行OpenShift Ansible脚本。对剧本使用`nohup`会创建文件描述符，
但不会关闭。因此，系统可能会耗尽要打开的文件，playbook也会失败。
====

要运行基于RPM的安装程序:

. 切换到playbook目录并运行*_prerequisites.yml_*剧本。这个剧本安装所需的软件包(如果有的话)，
并修改容器运行时。除非需要配置容器运行时，否则在第一次部署集群之前，只运行此剧本一次:
+
----
...
安装程序映像可以用作链接:
https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux_atomic_host/7/html/managing_containers/running_system_containers[系统容器].
系统容器存储并运行在传统的*docker*服务之外。这允许从目标主机之一
运行安装程序映像，而不需要关心主机上的安装重新启动*docker*。

要使用原子CLI作为一次运行的系统容器运行安装程序，请作为*root*用户执行以下步骤:

. 运行*_prerequisites.yml_*剧本:
+
----
...
第一次运行这个命令时，将图像导入
链接:https://access.redhat.com/documentation/en-us/red_hat_satellite/6.2/html/content_management_guide/managing_ostree_content[OSTree]
...
您可以使用`PLAYBOOK_FILE`环境变量来指定要使用容器化安装程序运行的其他剧本。
`PLAYBOOK_FILE`的默认值是
*_/usr/share/ansible/openshift-ansible/playbooks/deploy_cluster.yml_*, 
它是主集群安装剧本，但是您可以将它设置为容器内另一个剧本的路径。

例如，要在安装之前运行
xref:configuring-cluster-pre-install-checks[pre-install checks]剧本，
请使用以下命令：
...
. 接下来，运行*_deploy_cluster.yml_*剧本启动集群安装:
+
----
$ docker run -t -u `id -u` \
...
要在现在OpenStack上安装{product-title}，请使用OpenStack剧本。
有关剧本的更多信息，包括详细的先决条件，请参见
...
安装程序使用模块化的剧本，以便管理员可以根据需要安装特定的组件。通过分解角色和剧本，
可以更好地针对特定的管理任务。这将提高安装期间的控制水平，并节省时间。

主安装剧本*_{pb-prefix}playbooks/deploy_cluster.yml_*以特定的顺序运行一组单独的组件剧本，
...
. 检查xref:installer-known-issues[已知问题]，以检查任何特定的指令或工作区。
...
干净的系统安装，或者xref:../install/uninstalling_cluster.adoc#uninstalling[卸载]
并重新安装。
** 如果使用虚拟机，请从一个新映像或
...
. 重试安装:
** 您可以重新运行*_deploy_cluster.yml_*剧本了。
** 您可以运行其余的单独安装剧本。
+
如果您只想运行其余的剧本，首先运行失败阶段的剧本，然后按顺序运行其余的剧本。
用以下命令运行每个剧本:
+
----
...
有关详细信息，请查看https://github.com/kubernetes/kubernetes/issues/10030。

== 接下来是什么?

现在您有了一个工作的{product-title}实例，您可以:

- 部署一个xref:../install_config/registry/index.adoc#install-config-registry-overview[集成容器映像注册表]。
- 部署一个xref:../install_config/router/index.adoc#install-config-router-overview[路由器]。
ifdef::openshift-origin[]
