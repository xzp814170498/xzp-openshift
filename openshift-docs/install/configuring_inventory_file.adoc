...
安装playbooks读取您的目录文件，以及了解在何处如何跨主机安装{product-title}。

[NOTE]
====
请查看
link:https://docs.ansible.com/ansible/2.4/intro_inventory.html[Ansible文件]
有关目录文件格式的详细信息，包括有关的基本信息:
...
下面有几节描述是需要在您安装集群期间需要设置的常用变量文件。其中很多可能的变量是可选的。
对于开发环境，您可以接受默认值，但是在生产环境中您必须选择适当的值。

您可以查看
xref:example_inventories.adoc#install-config-example-inventories[示例目录文件]
可以作为集群安装的起点。
...
a| 默认情况下为 `true`，指是否在集群节点上启用网络时间协议(NTP)。
...
a|该变量根据目录主机文件中的要求设置参数和任意JSON值。例如：
...
|这个变量覆盖集群的主机名，它默认为主机名。

|`openshift_master_cluster_public_hostname`
|此变量覆盖集群的公共主机名，它默认为主机名。如果使用外部负载平衡器，请指定外部负载平衡器的地址。
...
* 如果 `openshift_image_tag` 被设置，它的值将用于系统容器环境中的所有主机，甚至包括那些安装了其他版本的主机。
* 如果 `openshift_pkg_version` 被设置，它的值被用于基于RPM的环境中的所有主机，即使是那些安装了其他版本的主机。
====
...
安装程序在整个playbooks和角色中使用的各种缺省值都基于
...
如果您没有在目录文件的 `[OSEv3:vars]` 组中设置 `openshift_node_groups` 变量，就使用这些默认值。
...
在安装期间，确保您不仅拥有专用的基础设施节点作为集群中定义的非主节点。
...
确保设置了所需的目录值，并确定主机上可能出现的问题，这些问题可以防止或干扰成功安装。
...
可以在目录文件中设置 `openshift_check_min_host_memory_gb`
集群变量，用来设置定义的最小内存需求值。
...
xref:../admin_guide/diagnostics_tool.adoc#admin-guide-health-checks-via-ansible-playbooks[Ansible-based健康检查]。
可以在其中找到另一组检查证书过期的检查
...
变量提供您的密码。还必须将用户名作为
`oreg_auth_user` 参数值提供。如果使用默认注册表，请为该用户指定密码或令牌.
|===

[NOTE]
====
默认注册表需要一个身份验证令牌。有关更多信息，
请查看 xref:../install_config/configuring_red_hat_registry.adoc#install-config-configuring-red-hat-registry[访问和配置 Red Hat 注册表]
====
...
|这个布尔变量指定是否所有定义的OpenShift主机和 `pass:[*.cluster.local]` 
...
xref:../install_config/configuring_authentication.adoc#session-options[会话选项]
在目录文件中是可配置的。 默认情况下，Ansible会用生成的身份验证和加密机密填充一个 `sessionSecretsFile`
...
因此，安装程序和更新playbooks需要一个选项来启用NFS和核心基础设施组件的使用。
...
例如，云服务商可以有一个带有供应者 `kubernetes.io/gce-pd` 在GCE上，以及一个独立存储提供程序(如GlusterFS)可以有一个带有供应程序 `kubernetes.io/glusterfs` 的 `StorageClass`。
在这两个情况下，使用以下变量：
...
如果你不想安装OAB，在目录文件中设置 `ansible_service_broker_install` 参数值为 `false`:
...
一些Ansible playbooks bundles (APBs)也需要一个PV用于它们自己的部署。
...
通过修改目录文件 `[OSEv3:vars]` 节中的以下内容来设置源项目: 
...
技术预览
link:https://coreos.com/blog/introducing-operator-framework[运营商框架]
