...
链接:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html-single/installation_guide/index[Red Hat Enterprise Linux (RHEL) 7.4或更高版本]
...
- 包含*_/var/_*的文件系统至少40GB硬盘空间。图片:redcircle-1.png[]
- 包含*_/usr/local/bin/_*的文件系统的最小1 GB硬盘空间。
- 包含系统临时目录的文件系统的最小1GB硬盘空间。图片:redcircle-2.png[]
...
ifdef::openshift-origin[]
* 相应操作系统: Fedora 21, CentOS 7.4,
链接:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html-single/installation_guide/index[RHEL 7.4或更高版本]
与"最小"安装选项，或者
链接:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux_atomic_host/7/html-single/installation_and_configuration_guide/[RHEL原子主机]
7.4.5或更高版本。
endif::[]
ifdef::openshift-enterprise[]
* 相应操作系统:
链接:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html-single/installation_guide/index[RHEL 7.4 or later]
与"最小"安装选项，或者
链接:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux_atomic_host/7/html-single/installation_and_configuration_guide/[RHEL原子主机]
7.4.5或更高版本。
endif::[]
...
* 1 vCPU.
* 最少8GB内存
* 包含*_/var/_*的文件系统至少需要15GB的硬盘空间。图片:redcircle-1.png[]
* 包含*_/usr/local/bin/_*的文件系统的最小1GB硬盘空间
* 包含系统临时目录的文件系统的最小1GB硬盘空间。图片:redcircle-2.png[]
* 为Docker的后端存储，每个运行容器的系统至少增加15 GB的未分配空间
请查看xref:host_preparation.adoc#configuring-docker-storage[配置Docker存储]. 
根据节点上运行的容器的大小和数量，可能需要额外的空间。

|外部etcd节点
a|* 至少20 GB硬盘空间的etcd数据。
* 查看链接:https://github.com/coreos/etcd/blob/master/Documentation/op-guide/hardware.md#hardware-recommendations[CoreOS etcd文档的硬件推荐部分]
了解如何正确设置etcd节点的大小。
...
|===
图片:redcircle-1.png[] 要满足RHEL原子主机中的*_/var/_*文件系统大小要求，需要更改默认配置。
有关配置的说明，请参见
...
图片:redcircle-2.png[] 系统的临时目录是根据Python标准库中的
...
Practices for {product-title}主机的推荐实践]，用于性能指导。
...
有关更多信息，请参考
xref:index.adoc#sizing[Sizing Considerations]和
...
包括::install_config/persistent_storage/topics/glusterfs_prereqs_hardware.adoc[]
...
更多信息，包括`*GOMAXPROCS*`环境变量如何工作，请参见链接:
https://golang.org/pkg/runtime/#GOMAXPROCS[Go Language documentation]。
...
有关如何操作的说明，请参阅原子主机文档
链接:https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux_atomic_host/7/html-single/managing_containers/#using_the_overlay_graph_driver[Overlay Graph Driver]
...
DNS的通配符最终必须解析为{product-title}xref:../architecture/networking/routes.adoc#routers[router]的IP地址。
...
xref:../architecture/infrastructure_components/kubernetes_infrastructure.adoc#high-availability-masters[多个master用于高可用性]使用标准集群安装过程，
您还必须选择一个IP来配置为您的xref:../admin_guide/high_availability.adoc#virtual-ips[virtual IP] (VIP)
在安装过程中使用。
...
您可以在
xref:configuring_inventory_file.adoc#advanced-install-configuring-firewalls[Ansible inventory文件]
...
* 对于Amazon Web服务，请参见xref:../install_config/configuring_aws.adoc#configuring-aws-permissions[Permissions]和
xref:../install_config/configuring_aws.adoc#configuring-a-security-group-aws[配置安全组]部分。
* 对于OpenStack，请参见xref:../install_config/configuring_openstack.adoc#configuring-openstack-permissions[Permissions]和
...
一些部署要求用户覆盖检测到的主机名和主机的IP地址。要查看默认值，请切换到playbook目录并运行`*openshift_facts*` playbook:
...
[IMPORTANT]
====
对于Amazon Web服务，请参见xref:../install_config/configuring_aws.adoc#overriding-detected-ip-addresses-host-names-aws[覆盖检测到的IP地址和主机名]部分。
====
...
The
xref:../install/configuring_inventory_file.adoc#configuring-ansible[配置您的库存文件]
